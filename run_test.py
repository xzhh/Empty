...
if bool_rdf:
  rdf_dr = 0.5*Lx/float(rdf_bin);
  for i in range( len(rdf_array_total) ):
      rdf_rad=(i+0.5)*rdf_dr
      rdf_fac=2.0*3.1415926536*rdf_rad*rdf_rad*rdf_dr*(num_particles/3.0)
      print("RDF: ", rdf_rad, rdf_array_total[i]/float(rdf_nlast)/rdf_fac/density*3.0)

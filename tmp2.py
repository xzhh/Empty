...
if not skipPPA:
  ...
  for i in range(num_chains):
    ...
    if rlen[i]>clen[i]:
      print("chain-",monomers_per_chain*i+1,"-",monomers_per_chain*(i+1),"> [",conf[0][monomers_per_chain*i+1],"|",conf[0][monomers_per_chain*(i+1)],"] ",rlen[i],clen[i],dim)
  ...
  print("ShearOffset = ",system.shearOffset)

...
def computeRDF(system, rdf_pair, rdf_dr, num_particles, Lx, Ly, Lz, types, atomtypes, num_types):
    ...
    for p in range(len(rdf_pair)):
        ...
        istart=type_end[itype-1]
        ilast=type_end[itype]
        
        for i in range(istart,ilast):
            idx=list_t[i]
            
            jlast=type_end[jtype]
            jstart=type_end[jtype-1]
    ...
    for p in range(len(rdf_pair)):
        idx=rdf_pair[p][0]
        jdx=rdf_pair[p][1]
        itot_f=float(type_end[idx]-type_end[idx-1])
        jtot_f=float(type_end[jdx]-type_end[jdx-1])
        ...
        for i in range(rdf_bin):
            ...
            out_stream.write("%.4f\t%.6f" %(rdf_rad, rdf_array[p][i]/float(rdf_nlast)/rdf_fac))
        out_stream.close()
        print("g(r) for %s-%s pairs is computed. See in %s" % (typeI,typeJ,filename))
...
rdf_pair = [[0,0],[0,1],[1,1]] # type-type [(H,H),(H,O),(O,O)]

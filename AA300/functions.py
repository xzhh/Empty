...
def write_h5md(file, system, step, time, record_time, temperature, energy):
    ...
    xbox, ybox, zbox = system.bc.boxL[0], system.bc.boxL[1], system.bc.boxL[2]
    ...
    for pid in range(1,max_pid+1):
        ...
        xpos, ypos, zpos = particle.pos[0], particle.pos[1], particle.pos[2]
        vx, vy, vz = particle.v[0], particle.v[1], particle.v[2]
        ...
        with h5py.File(file, 'r+') as h5:
            h5['particles/index'][iteration, pid-1, :] =  [molecule_number, molecule_name, atom_id_in_molecule]
            h5['particles/position'][iteration, pid-1, :] = [round(xpos,3), round(ypos,3), round(zpos,3)]
            h5['particles/velocity'][iteration, pid-1, :] = [round(vx,3), round(vy,3), round(vz,3)]
    ...
    with h5py.File(file, 'r+') as h5:
        ...
        h5['box/size'][:] = [xbox, ybox, zbox] #[Lx, Ly, Lz]
        
        h5['observables/temperature'][iteration] = round(temperature,3)
        h5['observables/time'][iteration] = time
        h5['observables/step'][iteration] = step
        
        for k in range(len(energy)): energy[k] = round(energy[k],3)

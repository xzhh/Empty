...
density            = 0.849
bondlen            = 0.97
monomers_per_chain = 20      # the number of monomers per a chain
...
for step in range(prod_nloops+1):
  if step == 0:
    ...
    for k in range(num_chains):
      ...
      pos_cur.append(system.storage.getParticle(cid1).pos)
      pos_sav.append(espressopp.Real3D(0.0, 0.0, 0.0))
      if monomers_per_chain%2==0:
        cid2=cid1+1
        l=wrap(system.storage.getParticle(cid1).pos,system.storage.getParticle(cid2).pos,Lx,Ly,Lz,system.shearOffset)
        for i in range(3):
          pos_cur[k][i]+=l[i]/2.0
      #determine the xyz of CoM
      ptmp=getCOM(system.storage,k*monomers_per_chain+1,monomers_per_chain,Lx,Ly,Lz,system.shearOffset)
  ...
  if step > 0:  
    ...
    for k in range(num_chains):
      ...
      if monomers_per_chain%2==1:
        ...
        cid1=int(monomers_per_chain*k+monomers_per_chain/2+0.00001)
      p=system.storage.getParticle(cid1).pos
      for i in range(3): pos_cur[k][i]=p[i]
      if monomers_per_chain%2==0:
        cid2=cid1+1
        l=wrap(system.storage.getParticle(cid1).pos,system.storage.getParticle(cid2).pos,Lx,Ly,Lz,system.shearOffset)
        for i in range(3):
    ...
    if step%nstep_div100==0:
      ...
      for i in range(num_chains):
        ...
        for j in range(1,monomers_per_chain):
          ...
          l=system.storage.getParticle(jj+1).pos-system.storage.getParticle(jj).pos
          if l[2]<-Lz/2.0:
            while l[2]<-Lz/2.0:
              dim[2]+=1
              l[2]+=Lz
        ...
        rtmp=system.storage.getParticle(monomers_per_chain*(i+1)).pos-system.storage.getParticle(monomers_per_chain*i+1).pos

...
def write_g96(filename, system, typenames, time, step, shear_rate=0):
    # only use for the CG model of BMIM-PF6
    BMI = "BMI"
    PF6 = "PF6"
    out_stream = open(filename, 'w')
    out_stream.write(f"TITLE\n")
    out_stream.write(f"CG BMIM-PF6 t={time:10.5f} step= {step} \n")
    ...
    for i in range(max_pid):
        ...
        zpos   = particle.pos[2]
        if particle.type != 4:
            out_stream.write(f"{mol_num:>5} {BMI:<5} {typenames[particle.type]:<5} "
                             f"{pid:>6}{xpos:15.9f}{ypos:15.9f}{zpos:15.9f}\n")
            if particle.type == 3:
                mol_num += 1
        else: 
            out_stream.write(f"{mol_num:>5} {PF6:<5} {typenames[particle.type]:<5} "
                             f"{pid:>6}{xpos:15.9f}{ypos:15.9f}{zpos:15.9f}\n")
            mol_num += 1 
    out_stream.write(f"END\n")
    out_stream.write(f"BOX\n")
    
    xbox = system.bc.boxL[0]
    ybox = system.bc.boxL[1]
    zbox = system.bc.boxL[2]
    shearbox = shear_rate*time*system.bc.boxL[0]
    out_stream.write(f"{xbox:15.9f}{ybox:15.9f}{zbox:15.9f}"
                     f"{0:15.9f}{0:15.9f}{0:15.9f}"
                     f"{0:15.9f}{shearbox:15.9f}{0:15.9f}\n")
    out_stream.write(f"END\n")
    out_stream.close()       
    
def write_g96_vel(filename, system, typenames, time, step):
    # only use for the CG model of BMIM-PF6
    BMI = "BMI"
    PF6 = "PF6"
    out_stream = open(filename, 'w')
    ...
    for i in range(max_pid):
        ...
        if particle.type != 4:
            out_stream.write(f"{mol_num:>5} {BMI:<5} {typenames[particle.type]:<5} "
                             f"{pid:>6}{xvel:15.9f}{yvel:15.9f}{zvel:15.9f}\n")
            if particle.type == 3:
                mol_num += 1
        else: 
            out_stream.write(f"{mol_num:>5} {PF6:<5} {typenames[particle.type]:<5} "
    ...
    out_stream.write(f"END\n")
    out_stream.write(f"BOX\n")
    out_stream.write(f"{1:15.9f}{1:15.9f}{1:15.9f}"
                     f"{0:15.9f}{0:15.9f}{0:15.9f}"
                     f"{0:15.9f}{0:15.9f}{0:15.9f}\n")
    out_stream.write(f"END\n")
    out_stream.close()       
...
if bool_rdf: 
    rdf_pair = [[0,0], [0,1], [0,2], [0,3], [0,4],
                ...
                [2,2], [2,3], [2,4],
                [3,3], [3,4],
                [4,4]] # input wanted type pairs (starting from Type-0 ...)
...
if not USE_EWALD:  #truncated coulomb
    ...
    alphaEwald     = 2.082275 #2.455 #np.exp(1.4548-1.0867*np.log(rspacecutoff))
...
sys.stdout.write(fmt % (0, T/constants.k/constants.N_A*1000, P, Pij[3], Etotal, Ek, Ep, EQQ, Eb, Ea, Ed))
...
out_energy.write(f"t T SIGXY etotal ekinetic epair ecoul ebond eangle edihedral\n")
out_energy.write(f"0 {T/constants.k/constants.N_A*1000} 0 {Etotal} {Ek} {Ep} {EQQ} {Eb} {Ea} {Ed} \n")
...
for step in range(prod_nloops+1):
    ...
    #for dih in dihedralinteractions.values(): Ed+=dih.computeEnergy()
    Etotal = Ek + Ep + EQQ + Eb + Ea + Ed
    sys.stdout.write(fmt % (step*(prod_isteps), T/constants.k/constants.N_A*1000, P, Pij[3], Etotal, Ek, Ep, EQQ, Eb, Ea, Ed))
    
    if (shear_rate>0.0):
        print("SIGXZ> %d %.6f" % (step*prod_isteps,system.sumP_xz))
        out_energy.write(f"{currentStep*timestep} {T/constants.k/constants.N_A*1000} {system.sumP_xz} {Etotal} {Ek} {Ep} {EQQ} {Eb} {Ea} {Ed} \n")
    else:
        out_energy.write(f"{currentStep*timestep} {T/constants.k/constants.N_A*1000} 0 {Etotal} {Ek} {Ep} {EQQ} {Eb} {Ea} {Ed} \n")

    #espressopp.tools.pdb.pdbwrite("traj.pdb", system, append=True, typenames={0:'O', 1:'N', 2:'C', 3:'S', 4:'P'})
    #sys.stdout.write('\n')
    if step%10 == 0:
        write_g96("out/out_" + str(integrator2.step).zfill(12) + ".g96", system, 
                ["I1","I3","I2","CT","PF"], integrator2.step * timestep, integrator2.step,
                shear_rate)
        write_g96_vel("out/v_out_" + str(integrator2.step).zfill(12) + ".g96", system, 
                ["I1","I3","I2","CT","PF"], integrator2.step * timestep, integrator2.step)
        
        os.system("gmx trjconv -f out/out_" + str(integrator2.step).zfill(12) + ".g96 -o out/out_" + str(integrator2.step).zfill(12) + ".xtc -ndec 4 -t0 " 
                + str(integrator2.step * timestep) + " -quiet yes")
        os.system("gmx trjconv -f out/v_out_" + str(integrator2.step).zfill(12) + ".g96 -o out/v_out_" + str(integrator2.step).zfill(12) + ".xtc -ndec 4 -t0 " 
                + str(integrator2.step * timestep) + " -quiet yes")
              
    if step%1000 == 0:
        if first_traj == True:
            os.system("gmx trjcat -f out/out_*.xtc -o out/traj.xtc -quiet yes")
            os.system("gmx trjcat -f out/v_out_*.xtc -o out/velocities.xtc -quiet yes")
            first_traj = False
        else:
            os.system("gmx trjcat -f out/out_*.xtc out/traj.xtc -o out/traj.xtc -quiet yes -nobackup")
            os.system("gmx trjcat -f out/v_out_*.xtc out/velocities.xtc -o out/velocities.xtc -quiet yes -nobackup")
        os.system("rm out/out_*.g96") 
        os.system("rm out/out_*.xtc") 
        os.system("rm out/v_out_*.g96") 
        os.system("rm out/v_out_*.xtc")
        out_energy.close()
...
os.system("gmx trjcat -f out/out_*.xtc out/traj.xtc -o out/traj.xtc -quiet yes -nobackup")
os.system("echo 0 | gmx trjconv -f out/traj.xtc -s start.gro -o out/traj_nojump.xtc -pbc nojump -quiet yes")
os.system("gmx trjcat -f out/v_out_*.xtc out/velocities.xtc -o out/velocities.xtc -quiet yes -nobackup")
os.system("rm out/out_*.g96") 
os.system("rm out/out_*.xtc") 
os.system("rm out/v_out_*.g96") 

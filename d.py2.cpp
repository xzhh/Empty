...
DomainDecomposition::DomainDecomposition(shared_ptr<System> _system, const Int3D& _nodeGrid,
                                         const Int3D& _cellGrid, int _halfCellInt)
    : Storage(_system, _halfCellInt), exchangeBufferSize(0) {
  LOG4ESPP_INFO(logger, "node grid = " << _nodeGrid[0] << "x" << _nodeGrid[1] << "x" << _nodeGrid[2]
                                       << " cell grid = " << _cellGrid[0] << "x" << _cellGrid[1]
                                       << "x" << _cellGrid[2]);
          ...
          for (int q = n - halfCellInt; q <= n + halfCellInt; ++q) {
            for (int r = m - halfCellInt; r <= m + halfCellInt; ++r) {
              if (p != o || q != n || r != m) {
                ...
                LOG4ESPP_TRACE(logger, "neighbor cell "
                                           << cell2 - getFirstCell() << " @ " << r << " " << q
                                           << " " << p << ((cell2Idx < cellIdx) ? " is" : " is not")
                                           << " taken");
              }
            }
          }
...
void DomainDecomposition::remapNeighbourCells(int cell_shift) {
  ...
  /*Particle *p1,*p2;
  int ri[20]={16490,16491,27495,2953,2954,4455};//{15213,15221,5379 ,4470 ,26266,4513 ,5762
  ,20380,18352,7019 ,39878,39873}; for (int j=0;j<6;j++){ p2 = lookupRealParticle(ri[j]); if (p2) {
  for (int cj=0;cj<215;cj++){
  cell2=&cells[cj];
  for (int ck=0;ck<cell2->particles.size();ck++) if (cell2->particles[ck].id()==ri[j])
  std::cout<<"pid("<<p2->id()<<"||"<<p2->position()<<") in rank-"<<getSystem()->comm->rank()<<" and
  in REAL cell "<<cj<<"-"<<ck<<" \n";
  }}}*/
...
void DomainDecomposition::decomposeRealParticles() {
  ...
  // if (rename("FLAG_P","FLAG_P")==0 && getSystem()->comm->rank()%16==0)//getSystem()->irank)
  // std::cout<<" FOLD-"<<getSystem()->comm->rank()<<"> "<<getSystem()->ghostShift<<"\n";
  bool allFinished;
    ...
    for (int coord = 0; coord < 3; ++coord) {
      ...
      if (nodeGrid.getGridSize(coord) > 1) {
        if (offs > .0 && coord == 2 &&
          ...
          if (nodeGrid.getNodePosition(2) == 0) {
            ptmp1 = (static_cast<int>(floor(
                         ((nodeGrid.getNodePosition(0) + .0) / (getInt3DNodeGrid()[0] + .0) * Lx +
                          offs) /
            ...
            ptmp1 = (static_cast<int>(floor(
                         ((nodeGrid.getNodePosition(0) + 1.0) / (getInt3DNodeGrid()[0] + .0) * Lx -
                          offs) /
                         Lx * getInt3DNodeGrid()[0])) %
                         getInt3DNodeGrid()[0] +
                     getInt3DNodeGrid()[0]) %
          ...
          // getSystem()->comm->rank()%16==0)//==getSystem()->irank) std::cout<<"
          // NODE(LR)-"<<getSystem()->comm->rank()<<"> "
          //<<node1<<" "<<node2<<" | "
          //<<nodeGrid.getNodeNeighborIndex(2 * coord)<<" "<<nodeGrid.getNodeNeighborIndex(2 *
          //coord+1)<<" \n";
            ...
            for (size_t p = 0; p < cell.particles.size(); ++p) {
              ...
              if (pos[coord] - cellGrid.getMyLeft(coord) < -ROUND_ERROR_PREC) {
                ...
                if (nodeGrid.getNodePosition(2) == 0) {
                  ...
                  if (abs(part.position()[0] - mid_1) < abs(part.position()[0] - mid_2))
          ...
          // if (rename("FLAG_P","FLAG_P")==0 &&
          // getSystem()->comm->rank()%16==0)//==getSystem()->irank) std::cout<<"
          // SEND/RECV-"<<getSystem()->comm->rank()<<"> \n";
          //<<sendBufL.size()<<" "<<sendBufR.size()<<" ("
          //<<sendBufL2.size()<<" "<<sendBufR2.size()<<") | "
          //<<recvBufL.size()<<" "<<recvBufR.size()<<" ("
          //<<recvBufL2.size()<<" "<<recvBufR2.size()<<") \n";
          // Exchange particles, odd-even rule
          int nsd, nrv;
          if (nodeGrid.getNodePosition(2) == 0) {
            nsd = node1;
            nrv = nodeGrid.getNodeNeighborIndex(2 * coord + 1);
          } else {
            nsd = nodeGrid.getNodeNeighborIndex(2 * coord);
            nrv = node1;
          }

          if (nodeGrid.getNodePosition(coord) % 2 == 0) {
            sendParticles(sendBufL, nsd);
            recvParticles(recvBufR, nrv);
            sendParticles(sendBufR, nrv);
            recvParticles(recvBufL, nsd);
          } else {
            recvParticles(recvBufR, nrv);
            sendParticles(sendBufL, nsd);
            recvParticles(recvBufL, nsd);
...
if (rename("FLAG_P","FLAG_P")==0 && getSystem()->shearOffset>36.095){
std::cout<<" TEST-"<<getSystem()->comm->rank()<<"> "
<<node1<<" "<<node2<<" "<<nodeGrid.getNodeNeighborIndex(2 * coord + 1)<<" "
<<"("<<sendBufL.size()<<" "<<sendBufR.size()<<"|"
<<recvBufL.size()<<" "<<recvBufR.size()<<"|"
<<sendBuf2.size()<<" "<<recvBuf2.size()<<")"<<" \n";
...
if (rename("FLAG_P","FLAG_P")==0 && getSystem()->shearOffset>36.095){
std::cout<<" TeST-"<<getSystem()->comm->rank()<<"> "
<<node1<<" "<<node2<<" "<<nodeGrid.getNodeNeighborIndex(2 * coord)<<" "
<<"("<<sendBufL.size()<<" "<<sendBufR.size()<<"|"
<<recvBufL.size()<<" "<<recvBufR.size()<<"|"
<<sendBuf2.size()<<" "<<recvBuf2.size()<<")"<<" \n";
throw std::runtime_error("STOP HERE!");
          ...
          if (nodeGrid.getNodePosition(2) == 0) {
            ...
            if (appendParticles(recvBuf2, 2 * coord + 1) && coord == 2) finished = false;
          }
          ...
          // if (rename("FLAG_P","FLAG_P")==0 &&
          // getSystem()->comm->rank()%16==0)//==getSystem()->irank) std::cout<<"
          // DRP4-"<<getSystem()->comm->rank()<<"> \n";
        } else {
          // if (rename("FLAG_P","FLAG_P")==0 && offs>.0 && coord==2 &&
          // getSystem()->comm->rank()%16==0)//getSystem()->irank) std::cout<<"
          // Intern-"<<getSystem()->comm->rank()<<"> "<<getSystem()->shearOffset<<"\n";
          for (std::vector<Cell*>::iterator it = realCells.begin(), end = realCells.end();
               it != end; ++it) {
            Cell& cell = **it;
        ...
        // if (rename("FLAG_P","FLAG_P")==0 && offs>.0 && coord==2 &&
        // getSystem()->comm->rank()%16==0)//getSystem()->irank) std::cout<<"
        // intern-"<<getSystem()->comm->rank()<<"> "<<getSystem()->ghostShift<<"\n";
      } else {
        ...
        if (offs > .0) {
          ...
          for (std::vector<Cell*>::iterator it = realCells.begin(), end = realCells.end();
            ...
            for (size_t p = 0; p < cell.particles.size(); ++p) {
              ...
              // part.id()==atoi(getenv("VAR1"))) std::cout<<" FOLD-"<<getSystem()->comm->rank()<<">
              // "<<part.id()<<" ("<<part.position()<<") \n";
              getSystem()->bc->foldCoordinate(part.position(), part.image(), coord);
              if (coord == 2 && ztmp != part.position()[2]) {
                real xtmp = part.position()[0] + offs * (part.position()[2] - ztmp) / Lz;
                int itmp = static_cast<int>(floor(xtmp / Lx));
                part.position()[0] = xtmp - (itmp + .0) * Lx;
              }
              // if (rename("FLAG_P","FLAG_P")==0 && getenv("VAR1")!=NULL &&
              // part.id()==atoi(getenv("VAR1"))) std::cout<<"   aft> "<<part.id()<<"
              // ("<<part.position()<<") \n";
    ...
    for (int lr = 0; lr < 2; ++lr) {
      ...
      if (nodeGrid.getGridSize(coord) == 1) {
        ...
        if (commCells[dir].ghosts.size() != commCells[dir].reals.size()) {
          throw std::runtime_error(
              "DomainDecomposition::doGhostCommunication: send/recv cell structure mismatch during "
              "local copy");
        }
        ...
        if (offs > .0 && coord == 2) {
          for (int i = 0, end = commCells[dir].ghosts.size(); i < end; ++i) {
            if (realToGhosts) {
              copyRealsToGhosts_LEBC(*commCells[dir].reals[i], *commCells[dir].ghosts[i], extradata,
                                     shift);
            } else {
        ...
        if (offs > .0 && coord == 2 &&
          ...
          int incell_shift =
              ((getSystem()->ghostShift) % getInt3DCellGrid()[0] + getInt3DCellGrid()[0]) %
              getInt3DCellGrid()[0];
          int new_dir = (nodeGrid.getNodePosition(2) > 0 ? -3 : -6) + dir;
          int sz01 = (getInt3DCellGrid()[1] + 2) * (getInt3DCellGrid()[0] + 1 - incell_shift);
          ...
          if ((new_dir > 0 ? new_dir : -new_dir) == 2) {
            if (dir == 4) {
              ...
              node1 = (getInt3DNodeGrid()[2] - 1) * getInt3DNodeGrid()[0] * getInt3DNodeGrid()[1];
            } else {
              shift_tmp = -getSystem()->ghostShift;
              node1 = 0;
            }
            ...
            throw std::runtime_error(
                "doGhostCommunication error: new_dir should be equal to one of [-2,-1,1,2] \n");
          if (shift_tmp >= 0) {
          ...
          // if (rename("FLAG_P","FLAG_P")==0 && getSystem()->ghostShift>0)
          // std::cout<<"NODE-"<<getSystem()->comm->rank()<<"> ["
          //<<node1<<","<<node2<<"] dir/opp/new: "<<dir<<" "<<oppositeDir<<" "<<new_dir<<" \n";
          ...
          for (int k = 0; k == 0 || (k == 1 && incell_shift > 0); k++) {
            if (sizesFirst) {
              ...
              if ((new_dir > 0 ? new_dir : -new_dir) == 2) {
                ...
                if (k == 0) {
                  ...
                  for (int i = 0, end = commCells[dir].reals.size(); i < end; ++i) {
                    sendSizes.push_back(commCells_bkp[dir - 4].reals[i]->particles.size());
                  }
                }
              ...
              // std::cout<<"  EXCHs> \n";
              if ((new_dir > 0 ? new_dir : -new_dir) == 2) {
                // if (incell_shift>0 && k==1)
                // std::cout<<" OUTWARD-"<<getSystem()->comm->rank()<<"> "
                //<<cnode[k]<<" "<<sendSizes.size()<<"("<<rsize[2]<<")"<<" | "
                //<<nodeGrid.getNodeNeighborIndex(oppositeDir)<<" "<<recvSizes.size()<<" ";
                // exchange sizes, odd-even rule
                if (nodeGrid.getNodePosition(coord) % 2 == 0) {
                  LOG4ESPP_DEBUG(logger, "sending to node " << cnode[k]);
                  getSystem()->comm->send(cnode[k], DD_COMM_TAG, &(sendSizes[0]), sendSizes.size());
                  if (k == 0) {
                ...
                // if (1==0 && dir==5 && incell_shift>0){
                // std::cout<<"  RANK-"<<getSystem()->comm->rank()<<"> ("
                //<<sendSizes.size()<<" "<<recvSizes.size()<<") ";
                // for (int i=rsize[k];i<rsize[k+1];i+=9) std::cout<<sendSizes[i-rsize[k]]<<" ";
                // std::cout<<"|";
                // for (int i=0;i<81 && k==0;i+=9) std::cout<<recvSizes[i]<<" ";
                // std::cout<<"\n";
                //}
              } else if ((new_dir > 0 ? new_dir : -new_dir) == 1) {
                // if (incell_shift>0 && k==1)
                // std::cout<<"  INWARD-"<<getSystem()->comm->rank()<<"> "
                //<<nodeGrid.getNodeNeighborIndex(dir)<<" "<<sendSizes.size()<<"("<<rsize[2]<<")"<<"
                //| "
                //<<cnode[k]<<" "<<recvSizes.size()<<" ";
                // exchange sizes, odd-even rule
                if (nodeGrid.getNodePosition(coord) % 2 == 0) {
                  ...
                  getSystem()->comm->recv(cnode[k], DD_COMM_TAG, &(recvSizes[0]), recvSizes.size());
                } else {
                  LOG4ESPP_DEBUG(logger, "receiving from node " << cnode[k]);
                  getSystem()->comm->recv(cnode[k], DD_COMM_TAG, &(recvSizes[0]), recvSizes.size());
                  if (k == 0) {
              ...
              if ((new_dir > 0 ? new_dir : -new_dir) == 2) {
                if (k == 0)
                  for (int i = 0, end = commCells_bkp[dir - 4].ghosts.size(); i < end; ++i) {
                    commCells_bkp[dir - 4].ghosts[i]->particles.resize(recvSizes[i]);
                  }
                LOG4ESPP_DEBUG(logger, "exchanging ghost cell sizes done");
              } else if ((new_dir > 0 ? new_dir : -new_dir) == 1) {
                for (int i = gsize[k], end = gsize[k + 1]; i < end; ++i) {
                  commCells[dir].ghosts[i]->particles.resize(recvSizes[i - gsize[k]]);
                }
                LOG4ESPP_DEBUG(logger, "exchanging ghost cell sizes done");
              }
              sendSizes.clear();
            ...
            if ((new_dir > 0 ? new_dir : -new_dir) == 2) {
              if (realToGhosts) {
                ...
                // absolute x-positions (counts in cells) of current real&ghost cells
                int cell_x1, cell_x2, itmp;
                real cx_flag = (shift[2] > .0 ? 1.0 : -1.0);
                for (int i = rsize[k], end = rsize[k + 1]; i < end; ++i) {
                  cell_x1 =
                      nodeGrid.getNodePosition(0) * getInt3DCellGrid()[0] +
                      (commCells[dir].reals[i] - getFirstCell()) % (getInt3DCellGrid()[0] + 2) - 1;
                  cell_x2 =
                      cnode[k] % getInt3DNodeGrid()[0] * getInt3DCellGrid()[0] +
                      (commCells[dir].ghosts[i] - getFirstCell()) % (getInt3DCellGrid()[0] + 2) - 1;
                  itmp = static_cast<int>(floor(
                      ((cell_x1 - cell_x2) * cellGrid.getCellSize(0) + cx_flag * offs) / Lx + 0.5));
                  packPositionsEtc_LEBC(outBuffer, *commCells[dir].reals[i], extradata, shift,
                                        cx_flag * offs - (itmp + .0) * Lx);
                  // if (1==1 && getSystem()->comm->rank()==getSystem()->irank)
                  // std::cout<<"  PACK-"<<getSystem()->irank<<"> "<<k<<" "<<cnode[k]<<" | C1C2 "
                  //<<cell_x1<<" "<<cell_x2<<" | "<<
                  // itmp<<" "<<cx_flag<<" "<<offs<<" "<<cx_flag*(offs+(itmp+.0)*Lx)<<" \n";
                }
              ...
              if (realToGhosts) {
                ...
                if (k == 0)
                  for (int i = 0, end = commCells_bkp[dir - 4].reals.size(); i < end; ++i) {
                    packPositionsEtc(outBuffer, *commCells_bkp[dir - 4].reals[i], extradata, shift);
                  }
                // if (getSystem()->comm->rank()==getSystem()->irank)
                // std::cout<<"  EXCHr> \n";
                // exchange particles, odd-even rule
                ...
                for (int i = gsize[k], end = gsize[k + 1]; i < end; ++i) {
                  ...
                  // if (getSystem()->comm->rank()%4==2 &&
                  // (commCells[dir].ghosts[i]-getFirstCell())%25==17 &&
                  //((getSystem()->comm->rank()==2 && commCells[dir].ghosts[i]-getFirstCell()>60) ||
                  //(getSystem()->comm->rank()==6 && commCells[dir].ghosts[i]-getFirstCell()<60))){
                  // std::cout<<" OUTB-"<<getSystem()->comm->rank()<<"> "
                  //<<commCells[dir].reals[i]-getFirstCell()<<"=>"
                  //<<commCells[dir].ghosts[i]-getFirstCell()<<" d"<<dir<<"/"<<new_dir<<" | ";
                  // ParticleList &pl=commCells[dir].ghosts[i]->particles;
                  // for(ParticleList::iterator p = pl.begin(), ed = pl.end(); p != ed; ++p)
                  // if (p->id()==2504 || p->id()==6411)
                  // std::cout<<p->id()<<" ("<<p->position()<<")";
                  // std::cout<<" \n";
                  //}
                }
            ...
            if ((new_dir > 0 ? new_dir : -new_dir) == 2) {
              if (realToGhosts) {
                // unpack received data
                // if (incell_shift>0 && rename("FLAG_P","FLAG_P")==0 &&
                // getSystem()->comm->rank()==getSystem()->irank && k==0) std::cout<<"
                // UNpos_INWA-"<<k<<"> ("<<getSystem()->comm->rank()<<") ";
                if (k == 0)
                  for (int i = 0, end = commCells_bkp[dir - 4].reals.size(); i < end; ++i) {
                    // if (incell_shift>0 && rename("FLAG_P","FLAG_P")==0 &&
                    // getSystem()->comm->rank()==getSystem()->irank && k==0)
                    // std::cout<<commCells_bkp[dir-4].ghosts[i]-getFirstCell()<<"("<<commCells_bkp[dir-4].ghosts[i]->particles.size();
                    unpackPositionsEtc(*commCells_bkp[dir - 4].ghosts[i], inBuffer, extradata);
                  }
                // if (incell_shift>0 && rename("FLAG_P","FLAG_P")==0 &&
                // getSystem()->comm->rank()==getSystem()->irank && k==0) std::cout<<" \n";
              } else {
                for (int i = rsize[k], end = rsize[k + 1]; i < end; ++i) {
                  ...
                  // if (getSystem()->comm->rank()%4==2 &&
                  // (commCells[dir].reals[i]-getFirstCell())%25==17 &&
                  //((getSystem()->comm->rank()==2 && commCells[dir].reals[i]-getFirstCell()>60) ||
                  //(getSystem()->comm->rank()==6 && commCells[dir].reals[i]-getFirstCell()<60))){
                  // std::cout<<" INBF-"<<getSystem()->comm->rank()<<"> "
                  //<<commCells[dir].ghosts[i]-getFirstCell()<<"<="
                  //<<commCells[dir].reals[i]-getFirstCell()<<" d"<<dir<<"/"<<new_dir<<" | ";
                  // ParticleList &pl=commCells[dir].reals[i]->particles;
                  // for(ParticleList::iterator p = pl.begin(), ed = pl.end(); p != ed; ++p)
                  // if (p->id()==2504 || p->id()==6411)
                  // std::cout<<p->id()<<" ("<<p->position()<<")";
                  // std::cout<<" \n";
              ...
              if (realToGhosts) {
                // if (incell_shift>0 && rename("FLAG_P","FLAG_P")==0 &&
                // getSystem()->comm->rank()==getSystem()->irank) std::cout<<"  UNpos_OUTW-"<<k<<">
                // ("<<getSystem()->comm->rank()<<") ";
                // unpack received data
                for (int i = rsize[k], end = rsize[k + 1]; i < end; ++i) {
                  // if (incell_shift>0 && rename("FLAG_P","FLAG_P")==0 &&
                  // getSystem()->comm->rank()==getSystem()->irank)
                  // std::cout<<commCells[dir].ghosts[i]-getFirstCell()<<"("<<commCells[dir].ghosts[i]->particles.size();
                  unpackPositionsEtc(*commCells[dir].ghosts[i], inBuffer, extradata);
                }
                // if (incell_shift>0 && rename("FLAG_P","FLAG_P")==0 &&
                // getSystem()->comm->rank()==getSystem()->irank) std::cout<<" \n";

                // if (1==0 && incell_shift>0){
                // std::cout<<"  UNPACK> ["
                //<<getSystem()->comm->rank()<<"] "
                //<<commCells[dir].ghosts[0]->particles[0].id()<<" ("
                //<<commCells[dir].ghosts[0]->particles[0].position()<<") \t"
                //<<commCells[dir].ghosts[sz01-1]->particles[0].id()<<" ("
                //<<commCells[dir].ghosts[sz01-1]->particles[0].position()<<") \t"
                //<<commCells[dir].ghosts[sz01]->particles[0].id()<<" ("
                //<<commCells[dir].ghosts[sz01]->particles[0].position()<<") \t"
                //<<commCells[dir].ghosts[commCells[dir].ghosts.size()-1]->particles[0].id()<<" ("
                //<<commCells[dir].ghosts[commCells[dir].ghosts.size()-1]->particles[0].position()<<")
                //"
                //<<" \n";
                //}
              } else {
                if (k == 0)
                  for (int i = 0, end = commCells_bkp[dir - 4].reals.size(); i < end; ++i) {
                    unpackAndAddForces(*commCells_bkp[dir - 4].reals[i], inBuffer);
                    // if (getSystem()->comm->rank()%4==2 &&
                    // (commCells[dir].reals[i]-getFirstCell())%25==17 &&
                    //((getSystem()->comm->rank()==2 && commCells[dir].reals[i]-getFirstCell()>60)
                    //|| (getSystem()->comm->rank()==6 &&
                    //commCells[dir].reals[i]-getFirstCell()<60))){ std::cout<<"
                    // INBF-"<<getSystem()->comm->rank()<<"> "
                    //<<commCells[dir].ghosts[i]-getFirstCell()<<"<="
                    //<<commCells[dir].reals[i]-getFirstCell()<<" d"<<dir<<"/"<<new_dir<<" | ";
                    // ParticleList &pl=commCells[dir].reals[i]->particles;
                    // for(ParticleList::iterator p = pl.begin(), ed = pl.end(); p != ed; ++p)

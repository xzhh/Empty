...
public:
    ...
    inline void freeBuf()
    {
        if (!owning || using_static_buff)
        {
            return ;
        }
        boost::alignment::aligned_free(buf);
        buf = staticBuf;
        using_static_buff = true;
        owning = true;
    }
    ...
    void allocate(size_t size)
    {
        if (owning){
            ...
            if (usedSize > capacity)
            {
                ...
                using_static_buff = false;
            }
        }
        else{
            throw std::runtime_error("Tried to allocate on a non owning buffer resize");
        }
    }
    ...
    [[nodiscard]] char* view(size_t start, size_t end) const
    {
    ...
    [[nodiscard]] int getUsedSize() const { return usedSize; }
    [[nodiscard]] char* getBuf() const { return buf; }

    void send(longint receiver, int tag) const { comm.send(receiver, tag, buf, usedSize); }

    [[nodiscard]] mpi::request isend(longint receiver, int tag) const
    {
        return comm.isend(receiver, tag, buf, usedSize);
    }

    void set_buffer(char* buffer,std::size_t new_capacity,bool own){
        owning = own;
        clear();
        buf = buffer;
        using_static_buff=false;
        usedSize = new_capacity;

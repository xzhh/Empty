...
DomainDecomposition::DomainDecomposition(shared_ptr<System> system,
    ...
    : baseClass(system, _nodeGrid, _cellGrid, _halfCellInt),
      subCellGrid({(std::size_t)_subCellGrid[0],(std::size_t) _subCellGrid[1],(std::size_t) _subCellGrid[2]}),
      ...
      HPX4ESPP_OPTIMIZE_COMM(_HPX4ESPP_OPTIMIZE_COMM), exchange_ghost_channels(nodeGrid, subNodeGrid)
{
    ...
    /// Fill in subnode parameters
    {
        ...
        for (size_t inode = 0; inode < virtualStorage.size(); inode++)
        {
            /// init cell grid info
            {
                ...
                const size_t gridNumCells = vd.cellGrid.getNumberOfCells(); // Number of cells in dimension (x*y*z)
...
void DomainDecomposition::updateGhostsBlocking()
{
    if (commUseChannels)
        ...
        ghostCommunication_impl<false, true, 0>(); // Comm1
}
...
void DomainDecomposition::collectGhostForcesBlocking()
{
    if (commUseChannels)
        ...
        ghostCommunication_impl<false, false, 0>(); // Comm2
}
        ...
        for (int lr = 0; lr < 2; ++lr)
        {
...

            {
                ...
                auto copyParticlesRealsToGhosts = [this, dir, extradata, &shift](int i) {
                    ...
                    for (auto src = reals.begin(), end = reals.end(),
                                                dst = ghosts.begin();
                         src != end; ++src, ++dst)
                    {
                ...
                else
                {
                    ...
                    longint totalSend = 0, totalRecv = 0;
                    {
                        ...
                        if (nodeGrid.getNodePosition(coord) % 2 == 0)
                        {
                            ...
                            hpx::util::format("DD 2601 Sending to {} recieveing from {}",nodeGrid.getNodeNeighborIndex(dir),nodeGrid.getNodeNeighborIndex(oppositeDir));

...
#SBATCH --ntasks-per-node=4   # only start 1 task via srun because Python multiprocessing starts more tasks internally
#SBATCH --cpus-per-task=18   # assign all the cores to that first task to make room for multithreading

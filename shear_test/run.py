...
if not USE_EWALD:  #truncated coulomb
    ...
    qq_interactions=gromacs.setCoulombInteractions(system, vl, rc, types, epsilon1=simulation_data.epsilon1, epsilon2=simulation_data.epsilon2, kappa=simulation_data.kappa) #for CG epsilon2=17.5, for atomistics 13 and for water 80
else:
    #define coulomb interactions with ewald
    coulomb_prefactor = simulation_data.coulomb_prefactor#138.935485
    rspacecutoff   = rc #3.0*pow(1/density,1.0/3.0) #  rspacecutoff - the cutoff in real space
    alphaEwald     = simulation_data.alphaEwald#2.885757 
    kspacecutoff   = simulation_data.kspacecutoff#15 #  kspacecutoff - the cutoff in reciprocal space
...
if not USE_DPD: # langevin thermostat
    ...
    langevin.gamma = simulation_data.Langevin_gamma#0.5
    langevin.temperature = simulation_data.Langevin_temperature#2.4942 # kT in gromacs units 
    ...
    dpd.gamma = simulation_data.DPD_gamma#1.0 # should be higher then langevin
    dpd.temperature = simulation_data.DPD_temperature#2.4942 # kT in gromacs units 
    integrator = espressopp.integrator.VelocityVerlet(system)
...
# print/record simulation parameters

if (simulation_type=='start'):

    with open('simulation_info.txt', 'w') as f:
        f.write('number of particles = %d' % num_particles + '\n')
        f.write('density = %.4f' % density + '\n')
        f.write('cut-off = %f' % rc + '\n')
        f.write('skin = %f' % (system.skin) + '\n')
        f.write('time step = %f' % (integrator.dt) + '\n')
        f.write('total time steps = %d' % (prod_nloops*prod_isteps) + '\n')
        f.write('NodeGrid = %s' % (nodeGrid,) + '\n')
        f.write('CellGrid = %s' % (cellGrid,) + '\n')
        f.write('random seed = %d' % irand + '\n\n')
        f.close()

elif (simulation_type == 'continue'):
    with open('simulation_info.txt', 'a') as f:
        f.write('random seed = %d' % irand + '\n\n')
        f.close()
...
for step in range(starting_frame, prod_nloops+1):
    ...
    P = 0
    Pij = [0,0,0,0,0,0]
    Ek = 0.0#0.5 * T * (3 * num_particles)
    N_particle = int(espressopp.analysis.MaxPID(system).compute()) #total number of particles
    for pid in range(1,N_particle+1):
        particle_info = system.storage.getParticle(pid)
        Ek += 0.5 * particle_info.mass * (particle_info.v[0]**2 + particle_info.v[1]**2 + particle_info.v[2]**2)
...
with open('simulation_info.txt', 'a') as f:
    f.write('random seed = %d' % irand + '\n')
    f.write('total # of neighbors = %d' % (vl.totalSize()) + '\n')
    f.write('average neighbors/atoms = %.1f' % (vl.totalSize()/float(num_particles)) + '\n')
    f.write('neighbor list builds = %d' % (vl.builds) + '\n')
    f.write('integration steps = %d' % (integrator2.step) + '\n\n')

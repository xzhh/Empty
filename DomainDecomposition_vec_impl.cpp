...
/// \return List where each pair forms a range, the last value is used to indicate the actual total size
...
//                std::cout<<"nodeGrid.getGridSize(coord) == 1" << std::endl;
            ...
            else
            {
             ...
             longint sendTotalBytes, recvTotalBytes;
                ...
                const auto channel_name = hpx::util::format("exchange/ghosts/impl/{}/{}/{}",lr,coord,iteration);

//                std::cout<<hpx::util::format("{} to {} : Sending Information lr={} coord={}",rank,nodeGrid.getNodeNeighborIndex(oppositeDir),lr,coord) << std::endl;
                ...
                auto my_recv_channel = hpx::find_from_basename<hpx::distributed::channel<std::vector<int>>>(channel_name,nodeGrid.getNodeNeighborIndex(oppositeDir));
                // TODO: FIX PERSISTENT CHANNELS
//                  auto& my_send_channel = exchange_ghost_channels.sendChannel(coord, lr);
//                  auto& my_recv_channel = exchange_ghost_channels.recvChannel(coord, lr);
                {
                    const real time = wallTimer.getElapsedTime();
                    ...
                    SPDLOG_TRACE("GHOSTExchangeimpl,Sender {} Reciever {}",nodeGrid.getNodeNeighborIndex(dir),
                            nodeGrid.getNodeNeighborIndex(oppositeDir));
                    spdlog::stopwatch sw;
                    auto send_fut = my_send_channel.set(hpx::launch::async,sendSizes);
                    hpx::future<std::vector<longint>> send_ranges_future = hpx::async(hpx::launch::async,sizes_to_ranges<ALIGNED>,sendSizes,sizeTotal);
            ...
            HPX4ESPP_DDVI_CHANNELS_DEBUG({
                ...
                if (!rank)
                    for (int ii = 0; ii < numSubNodes; ii++)
                    {
                        ...
                        if (std::get<0>(ns))
                        {
                            auto const& idx = exchange_ghost_channels.sendIdx(coord, lr, std::get<1>(ns));

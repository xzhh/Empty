...
public:
    ...
    void preset()
    {
        ...
        */
        ...
        if (atoi(getenv("FCS_NEAR")) == 1)
            strcat(common_parameters, ",near_field_flag,1");
        else
            strcat(common_parameters, ",near_field_flag,0");
    ...
    bool _computeForce(CellList realcells)
    {
        ...
        // cout<<"TUNE_FLAG: "<<ifTuned<<" "<<(ifTuned? 1:0)<<" \n";
        // cout<<"PRM: "<<p3m_alpha<<" "<<p3m_rcut<<" "<<p3m_cao<<" "<<p3m_grid<<" \n";
        // ifTuned=1;
        p3m_alpha = 2.45399;
        p3m_rcut = 0.9;
        p3m_cao = 7;
        ...
        if (ifTuned)
        {
            ...
            // fcs_p3m_set_r_cut(handle, p3m_rcut);
        ...
        // printf("EN-0 %f %f %f %f \n",en_local,sfcs_coor[0],*(sfcs_pot+0),sfcs_field[0]);
        // printf("EN-1 %f %f %f %f \n",en_local,sfcs_coor[1],*(sfcs_pot+1),sfcs_field[1]);
        // printf("EN-2 %f %f %f %f \n",en_local,sfcs_coor[2],*(sfcs_pot+2),sfcs_field[2]);
        free(sfcs_coor);
        free(sfcs_cg);
        free(sfcs_field);
        free(sfcs_pot);

source $HOME/bin/gnu.2020a
source $HOME/bin/cmk.load
module load lib/jemalloc/5.2.1-GCCcore-9.3.0 
module load system/hwloc/2.2.0-GCCcore-9.3.0
export HPX_DIR=/lustre/miifs01/project/m2_zdvresearch/zhexu/build-hpx-1.5.1-gcc9.3/local/lib64/cmake/HPX/
ESPP_EVAL_DIR="/lustre/miifs01/project/m2_zdvresearch/zhexu/build-h4e-intel2020-jv/evaluation"
export ESPPATH=/lustre/miifs01/project/m2_zdvresearch/zhexu/build-h4e-gcc9.3-jv/
source $HOME/bin/esprc
export PYTHONPATH="${PYTHONPATH}:${ESPP_EVAL_DIR}"
export LD_PRELOAD=/cluster/easybuild/broadwell/software/jemalloc/5.2.1-GCCcore-9.3.0/lib/libjemalloc.so
...
CMD_FLAGS="-p parallel -A m2_zdvresearch -C skylake"
SIM_ARGS=""
...
function run_cmd() {
    ...
    CMD="${CMD_PRE} ${CMD_FLAGS} -n ${NUM_RANKS} ${CMD_SLURM_PRE} python lj.py ${SIM_ARGS} ${EXE_ARGS} ${OUT_ARGS}"
    echo CMD=\"${CMD}\"
    echo
    eval $CMD
    echo

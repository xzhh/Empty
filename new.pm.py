...
#nodeGrid=espressopp.Int3D(8,2,2)
#cellGrid=espressopp.Int3D(15,11,11)
...
print("BoxL          = ", size)
print("RandomSeed    = ", system.rng.get_seed())
...
for step in range(warmup_nloops):
  ...
  potLJ.epsilon = (step+1.0)/warmup_nloops*1.0
  interLJ.setPotential(type1=0, type2=0, potential=potLJ)
  if (ifbond):
...
for step in range(prod_nloops+1):
  ...
  if step > 0:  
    ...
    if (math.isnan(interFENE.computeEnergy())):
      ...
      print("STEP(shear): ",step*prod_isteps,system.shearOffset)
      sys.exit(0)
      #conf.gather()
      for j in range(1,num_particles+1):
        #print("PID-%d: [%.3f,%.3f,%.3f] [%.3f,%.3f,%.3f] [%.3f,%.3f,%.3f]" %(j,conf[0][j][0],conf[0][j][1],conf[0][j][2],conf[1][j][0],conf[1][j][1],conf[1][j][2],conf[2][j][0],conf[2][j][1],conf[2][j][2]),flush=True)
        if j%monomers_per_chain!=0:
          l=conf[0][j+1]-conf[0][j]
          stmp=system.shearOffset
          if l[2]<-Lz/2.0:
            l[2]+=Lz
            l[0]+=stmp
          elif l[2]>Lz/2.0:
          ...
          if l[0]<-Lx/2.0:
            ...
            while l[0]>Lx/2.0: l[0]-=Lx
          if l.abs()>1.5:
            print("FENE-PAIR-%d-%d - %.4f [%.4f,%.4f,%.4f] [%.4f,%.4f,%.4f]" %(j,j+1,l.abs(),conf[0][j][0],conf[0][j][1],conf[0][j][2],conf[0][j+1][0],conf[0][j+1][1],conf[0][j+1][2]),flush=True)
            
            l=conf[1][j+1]-conf[1][j]
            stmp=system.shearOffset
            if l[2]<-Lz/2.0:
              l[2]+=Lz
              l[0]+=stmp
            ...
            if l[0]<-Lx/2.0: 
              ...
              while l[0]>Lx/2.0: l[0]-=Lx
            print("FENE-PAIR-%d-%d - %.4f [%.4f,%.4f,%.4f] [%.4f,%.4f,%.4f]" %(j,j+1,l.abs(),conf[1][j][0],conf[1][j][1],conf[1][j][2],conf[1][j+1][0],conf[1][j+1][1],conf[1][j+1][2]),flush=True)
            
            l=conf[2][j+1]-conf[2][j]
            stmp=system.shearOffset
            if l[2]<-Lz/2.0:
              l[2]+=Lz
              l[0]+=stmp
            ...
            if l[0]<-Lx/2.0:
              ...
              while l[0]>Lx/2.0: l[0]-=Lx
            print("FENE-PAIR-%d-%d - %.4f [%.4f,%.4f,%.4f] [%.4f,%.4f,%.4f]" %(j,j+1,l.abs(),conf[2][j][0],conf[2][j][1],conf[2][j][2],conf[2][j+1][0],conf[2][j+1][1],conf[2][j+1][2]),flush=True)
      sys.exit(0)
...
if not skipPPA:
  ...
  for i in range(num_chains):
    ...
    rlen=conf[0][monomers_per_chain*(i+1)]-conf[0][monomers_per_chain*i+1]
    rlen[0]+=dim[0]*Lx
    rlen[1]+=dim[1]*Ly
    rlen[2]+=dim[2]*Lz
    r2_sum+=rlen.abs()**2
  ...
  for i in range(num_chains):
    c2=clen[i]**2
    klen.append(r2_msq/clen[i])
    elen.append(r2_msq/c2*(monomers_per_chain-1))
    #tbd.append(math.sqrt(elen[i])*clen[i]/(monomers_per_chain-1))
    znum.append(monomers_per_chain/(monomers_per_chain-1)*(c2/r2_msq-1))
